    ...
    def forward(self, x, comp_drop=False):
        ...
        if comp_drop:
            ...
            for i in range(0, len(features)):
                ...
                dropout_mask1 = self.binomial.sample((bs // 2, d)).cuda() * 2.0
                dropout_mask2 = 2.0 - dropout_mask1
                dropout_mask1[kept_indexes, :] = 1.0
                dropout_mask2[kept_indexes, :] = 1.0
                dropout_mask_all = torch.cat((dropout_mask1, dropout_mask2))
                
                features[i] = features[i] * dropout_mask_all.unsqueeze(2).unsqueeze(3)         
...
class UNet_cps(nn.Module):
    ...
    def forward(self, x, comp_drop=None):
        features = self.encoder(x)
        if comp_drop != None:
            for i in range(0, len(features)):
                features[i] = features[i] * comp_drop[i].unsqueeze(2).unsqueeze(3)     
